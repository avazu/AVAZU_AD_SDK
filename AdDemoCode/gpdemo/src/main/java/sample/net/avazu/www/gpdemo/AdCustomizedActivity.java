...
public class AdCustomizedActivity extends Activity implements View.OnClickListener,CompoundButton.OnCheckedChangeListener{
	...
	private void showAdView(){
		...
		if (style == Utils.Constants.STYLE_BANNER){
			if (currentFormat == 0){
				show_type = AvazuAdView.SINGLE_BANNER;
			}else{
				show_type = AvazuAdView.MULTIPLE_LINE_BANNER;
			}
		}else{
			if (currentFormat == 0){
				show_type = AvazuAdView.SINGLE_LINE_RECTANGLE;
			}else{
				show_type = AvazuAdView.MULTIPLE_LINE_RECTANGLE;
			}
		}
		...
		adView.setAdInstallButtonBackgroundColor(TextUtils.isEmpty(mBbc1) ? mBbc1 : "#" + mBbc1);
		adView.setAdInstallButtonTextColor(TextUtils.isEmpty(mBtc) ? mBtc : "#" + mBtc);
		adView.loadWebviewAd(this);

		int width = width_dip > 0 ? Utils.dip2px(this, width_dip) : getResources().getDisplayMetrics().widthPixels;
		int height = height_dip > 0 ? Utils.dip2px(this, height_dip) : getResources().getDisplayMetrics().heightPixels;
