...
public class AdCustomizedActivity extends Activity implements View.OnClickListener,CompoundButton.OnCheckedChangeListener{
	...
	private void showAdView(){
		...
		AdViewSettings adSettings = new AdViewSettings(width_dip, height_dip, show_type, true);
		adSettings.setAppCount(appNums);
		adSettings.setNeedBtn(mIsNeedBtn);
		adSettings.setNeedCat(mIsNeedCat);
		adSettings.setNeedIcon(mIsNeedIcon);
		adSettings.setNeedInstalls(mIsNeedInstalls);
		adSettings.setNeedRating(mIsNeedRating);
		adSettings.setNeedReviewNum(mIsNeedReviewNums);
		adSettings.setNeedSize(mIsNeedSize);
		adSettings.setNeedTitle(mIsNeedTitle);
		adSettings.setMainBackColor(TextUtils.isEmpty(mWbc) ? mWbc : "#" + mWbc);
		adSettings.setBlockBackColor(TextUtils.isEmpty(mBbc) ? mBbc : "#" + mBbc);
		adSettings.setAppTitleColor(TextUtils.isEmpty(mAtc) ? mAtc : "#" + mAtc);
		adSettings.setButtonBackColor(TextUtils.isEmpty(mBbc1) ? mBbc1 : "#" + mBbc1);
		adSettings.setButtonTextColor(TextUtils.isEmpty(mBtc) ? mBtc : "#" + mBtc);
		AvazuAdView adView = new AvazuAdView(this, adSettings);
		adView.loadWebviewAd();
		int width = adSettings.getWidthDip() > 0 ? Utils.dip2px(this, adSettings.getWidthDip()) : getResources().getDisplayMetrics().widthPixels;
		int height = adSettings.getHeightDip() > 0 ? Utils.dip2px(this, adSettings.getHeightDip()) : getResources().getDisplayMetrics().heightPixels;
