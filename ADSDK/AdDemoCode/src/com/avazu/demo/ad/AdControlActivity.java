		...
		mShowTypeChoose.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(RadioGroup group, int checkedId) {
				...
				case R.id.show_type_rect_single:
					mShowType = AdViewController.TYPE_RECT_SINGLE;
					onSingleTypeChose();
					break;

				case R.id.show_type_rect_wall:
					mShowType = AdViewController.TYPE_RECT_WALL;
					onWallTypeChose();
					break;
					
				case R.id.show_type_banner_single:
					mShowType = AdViewController.TYPE_BANNER_SINGLE;
					onSingleTypeChose();
					break;
					
				case R.id.show_type_banner_wall:
					mShowType = AdViewController.TYPE_BANNER_WALL;
					onWallTypeChose();
					break;
				}
			}
...
//		mAppCountInput.setVisibility(View.INVISIBLE);
	...
	private boolean checkElementsSelect(){
		...
		if(!ret){
			Toast.makeText(this, "请至少选择一个广告内容元素!", Toast.LENGTH_SHORT).show();
		}
		if (mShowType == AdViewController.TYPE_BANNER_WALL || mShowType == AdViewController.TYPE_RECT_WALL){
			try {
				mAppCount = Integer.valueOf(mAppCountInput.getText().toString());
				if (mAppCount <= 0){
					ret = false;
					Toast.makeText(this, "请输入正确的广告数量!", Toast.LENGTH_SHORT).show();
				}
			} catch (Exception e) {
	...
	private boolean checkColorInput(){
		...
		if (!TextUtils.isEmpty(mWbc) && mWbc.length() != 6 && !mWbc.equals(noNeedSet)){
			Toast.makeText(this, "请输入正确格式的整体底色设置!", Toast.LENGTH_SHORT).show();
			mWbc = "";
			return false;
		}
		...
		if (!TextUtils.isEmpty(mBbc) && mBbc.length() != 6 && !mBbc.equals(noNeedSet)){
			Toast.makeText(this, "请输入正确格式的广告底色设置!", Toast.LENGTH_SHORT).show();
			mBbc = "";
			return false;
		}
		...
		if (!TextUtils.isEmpty(mAtc) && mAtc.length() != 6 && !mAtc.equals(noNeedSet)){
			Toast.makeText(this, "请输入正确格式的标题颜色设置!", Toast.LENGTH_SHORT).show();
			mAtc = "";
			return false;
		}
		...
		if (!TextUtils.isEmpty(mBbc1) && mBbc1.length() != 6 && !mBbc1.equals(noNeedSet)){
			Toast.makeText(this, "请输入正确格式的按钮底色设置!", Toast.LENGTH_SHORT).show();
			mBbc1 = "";
			return false;
		}
		...
		if (!TextUtils.isEmpty(mBtc) && mBtc.length() != 6 && !mBtc.equals(noNeedSet)){
			Toast.makeText(this, "请输入正确格式的按钮字色设置!", Toast.LENGTH_SHORT).show();
			mBtc = "";
			return false;
		}
		return true;
	...
	private void goShowAd(){
		if (mShowType == AdViewController.TYPE_RECT_WALL || mShowType == AdViewController.TYPE_BANNER_WALL){
			mAdWidth = -1;
			mAdHeight = -1;
		}
		AdViewController controller = new AdViewController(mAdWidth, mAdHeight, mShowType, true);
