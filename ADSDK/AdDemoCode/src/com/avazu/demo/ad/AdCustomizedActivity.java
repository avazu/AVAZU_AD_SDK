...
public class AdCustomizedActivity extends Activity implements View.OnClickListener,CompoundButton.OnCheckedChangeListener{
	...
	private void showAdView(){
		...
		if (style == Utils.Constants.STYLE_BANNER){
			if (currentFormat == 0){
				show_type = AdViewController.TYPE_BANNER_SINGLE;
			}else{
				show_type = AdViewController.TYPE_BANNER_WALL;
			}
		}else{
			if (currentFormat == 0){
				show_type = AdViewController.TYPE_RECT_SINGLE;
			}else{
				show_type = AdViewController.TYPE_RECT_WALL;
			}
		}
		AdViewController controller = new AdViewController(width_dip, height_dip, show_type, true);
